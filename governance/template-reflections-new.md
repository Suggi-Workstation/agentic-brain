...
> Changes: single compliance checklist annotated with quality gates
> (G1-G9), duplicate Quality Gates list retired, wording enriched from
> the write-reflection skill, header hierarchy made explicit.
...
this checklist in the published reflection. The G# labels (G1-G9)
are the named quality gates; they are enforced by this checklist.

- [ ] Frontmatter Schema complete: all 7 fields present (G8)  (PASS / HALT)
- [ ] Frontmatter Rules correctly applied: name=kebab-case, id=date-generated, tier=reflection, trigger=canonical-value, author=capitalized, tags=lowercase-hyphens, links=relative-paths. `brain:` prefix ONLY for cross-repo references; omit for same-repo links  (PASS / HALT)
- [ ] id generated by running `date -u +'%Y%m%dT%H%M%SZ'` (not estimated, not rounded); does not end in 000000Z (human-rounded = reject); never manually typed  (PASS / HALT)
- [ ] Title makes a claim (G1): something someone can agree or disagree with. Not "Notes on X" -- that is a draft  (PASS / HALT)
- [ ] I section: idea stated in one sentence + context (G2)  (PASS / HALT)
- [ ] O section: clear position + confidence level (G3): high 85%+ / medium 60-85% / low below 60%, with why  (PASS / HALT)
- [ ] R section: Surprise (30%) / Feel (30%) / Learn (40%) (G4)  (PASS / HALT)
- [ ] Surprise answers "I expected X, but Y happened"; if nothing surprised you, the reflection is incomplete  (PASS / HALT)
- [ ] One actionable change (G5): concrete, structural, executable -- another agent could execute it from the description alone. Not "be better" or "pay attention"  (PASS / HALT)
...
- [ ] Cross-links: at least 1 link to Library/insight/other reflection (G6). Zero links = dead-end knowledge  (PASS / HALT)
...
G1-G9 are the named subset of the Reflection Checklist above, annotated in
parentheses on the relevant items. The checklist is the single enforcement
gate; no separate gate list is maintained (R8: reference, never duplicate).
