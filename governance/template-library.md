...
- `author` is who wrote the topic (e.g. Link, Ava, Zelda, Suggi, Luffy).
...
- [ ] Frontmatter Schema complete (7 fields: name, id, tier, domain, author, tags, links)  (PASS / HALT)
- [ ] Frontmatter Rules correctly applied (7 fields: name, id, tier, domain, author, tags, links)  (PASS / HALT)
- [ ] id generated by running `date -u +'%Y%m%dT%H%M%SZ'` (not estimated, not rounded)  (PASS / HALT)
- [ ] Domain matches the folder the file lives in  (PASS / HALT)
- [ ] Tags include the file's type (concept, person, company, book, etc.)  (PASS / HALT)
- [ ] Hypothesis makes a claim (not a description)  (PASS / HALT)
- [ ] Analytical section: at least one structured element (table, formula, criteria, framework)  (PASS / HALT)
- [ ] Narrative section: qualitative explanation with examples or quotes  (PASS / HALT)
- [ ] Conclusion: restates hypothesis + one actionable takeaway + open questions  (PASS / HALT)
- [ ] Cross-links: at least 2 links to other brain content (library/reflections/insights/reports)  (PASS / HALT)
- [ ] Source citations: every factual claim traces to a primary source, data, or "author's analysis"  (PASS / HALT)
- [ ] Feynman test: a domain newcomer can follow from start to finish  (PASS / HALT)
- [ ] Filename: <domain>/<slug>.md -- lowercase, kebab-case  (PASS / HALT)
