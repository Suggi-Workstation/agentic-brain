...
the Writer skill, refreshed by the Reviewer skill, and proposed by the
...
- [ ] Frontmatter Schema complete: all 7 creation fields present (name, id, tier, domain, author, tags, links). `reviewed` OMITTED from the initial write (added later by Reviewer). (PASS / HALT)
...
- [ ] `## Background` section present with substantive content -- at least 600 words (G12) (PASS / HALT)
- [ ] `## Core Concepts` section present with essential ideas. Title MAY vary by domain (e.g. `## Core Biases by Category`, `## Core Principles`) but the section MUST exist. Contains substantive content -- at least 1200 words. (G12) (PASS / HALT)
- [ ] `## Evidence` section present with empirical support, research findings, or case studies. Title MAY vary (e.g. `## Evidence and Research Foundation`) but MUST exist. Contains substantive content -- at least 900 words. (G12) (PASS / HALT)
- [ ] `## Implications` section present -- why the topic matters, practical application. Contains substantive content -- at least 900 words. (G12) (PASS / HALT)
- [ ] Section minimums verified by word count on the extracted section text:
...
- [ ] Every factual claim traceable to a source in Sources. Synthesized claims are labeled as such. Citations follow the Inline citations rule; every cited number resolves to a numbered entry in `## Sources`. (G3) (PASS / HALT)
...
reviewed: <YYYY-MM-DD>             # added or updated by Reviewer. OMIT from the initial write.
---
```
...
- `domain` is the kebab-case domain slug matching the folder name.
  Must match a domain anchor listed in `library/index-library.md`.
  Example: `value-investing`, `psychology-behavior`, `history`.
...
- `reviewed` is added or updated by the Reviewer after a review cycle.
  Value: the UTC review date in `YYYY-MM-DD` format. OMIT from the
  initial write.
...
MUST produce every listed section. The Reviewer MUST preserve every
listed section when rewriting stale content.
...
- **G8 -- Frontmatter Complete:** All 7 required fields present (name,
  ...
  `date -u +'%Y%m%dT%H%M%SZ'` and is not human-rounded. `reviewed`
  OMITTED from the initial write (added later by Reviewer).
...
- **G12 -- Mandatory Section Quality:** Each mandatory section
  ...
  substantive, non-trivial content. Minimums: Background 600 words,
  Core Concepts 1200 words, Evidence 900 words, Implications 900
  words. Verified by word count on the extracted section text:
  `awk '/^## /{sec=$2; next} {c[sec]+=NF} END{for (s in c) print s, c[s]}' <topic-file>`.
  A section with only one sentence, a bare bullet list without
  explanation, or placeholder text fails this gate. The section
  must be worth reading independently -- if a reader skipped to
  this section, they would learn something. Word count alone does
  not pass this gate: the section must develop its concepts, not
  pad to reach the floor.
...
step 2). The Reviewer uses it as the format reference when refreshing
content. Changes to this file propagate to all future topics.*
