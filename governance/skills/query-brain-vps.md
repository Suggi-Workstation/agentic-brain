...
description: "Use when searching the shared agentic-brain index on the VPS."
user-invocable: true
disable-model-invocation: false
---

# Query-Brain-VPS -- Search the Agentic-Brain

## Purpose

Searches `agentic-brain` with the watcher-maintained VPS index. The engine runs
...
- The answer may exist in governance, Library topics, research, and reflections.
...
| Item | Path |
|:--|:--|
| Repository | `/srv/brain/agentic-brain` |
| Search tool | `brain-index/` |
| Index data | `~/.brain-index` (`/srv/brain/index`) |
| Index log | `/srv/brain/logs/brain-index.log` |
...
Run `crontab -l` and confirm the exact `agentic-brain` watcher line is present.
...
`NO INDEX`, or `UNVERIFIED`; inspect `/srv/brain/logs/brain-index.log` and report the fault.
...
Read the top 3-5 relevant files from `/srv/brain/agentic-brain/<returned-path>`. Snippets
...
`search_files` against `/srv/brain/agentic-brain` with an appropriate Markdown filter, then
...
- [ ] Query returned ranked paths from `agentic-brain` (PASS / HALT)
...
- `brain-index/README.md` -- engine and evaluation commands
