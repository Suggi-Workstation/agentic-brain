...
description: "Query the shared agentic-forge index on the fleet VPS -- no cloning, watcher-maintained freshness. Use for all forge searches by any fleet agent."
user-invocable: true
disable-model-invocation: false
---

# Query-Forge-VPS -- Search the Agentic-Forge (Shared VPS Index)

## What This Skill Does

Searches the agentic-forge knowledge base using the forge-index hybrid
...
- User asks a question that might be answered by brain content
  (protocol, research builds, proposals, validations, learnings)
...
```bash
/opt/brain-tools/repo-pull.sh /srv/forge/agentic-forge /srv/forge/logs /srv/forge/agentic-forge/forge-index forge && echo WATCHER_OK
```

### 2. Verify index freshness

```bash
cd /srv/forge/agentic-forge && /opt/brain-tools/venv/bin/python forge-index/query.py --check-freshness
```
...
/opt/brain-tools/venv/bin/python forge-index/query.py "<query>" --top-k 20
```
...
ssh -i <agent-key> -p 22 root@100.99.142.120 \
  "su - hermes -c \"cd /srv/forge/agentic-forge && /opt/brain-tools/venv/bin/python forge-index/query.py '<query>' --top-k 20\""
```
...
```bash
grep -r "<search terms>" /srv/forge/agentic-forge/ --include="*.md"
```
...
- **Run brain commands as hermes, not root.** The index path resolves
  via hermes's `~/.forge-index` symlink (`/srv/forge/index`); root has
...
- `forge-index/README.md` -- tool usage documentation
