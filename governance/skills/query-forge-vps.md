...
```bash
/opt/repo-tools/repo-pull.sh /srv/forge/agentic-forge /srv/forge/logs /srv/forge/agentic-forge/forge-index forge && echo WATCHER_OK
```

### 2. Verify index freshness

```bash
cd /srv/forge/agentic-forge && /opt/repo-tools/venv/bin/python forge-index/query.py --check-freshness
```
...
/opt/repo-tools/venv/bin/python forge-index/query.py "<query>" --top-k 20
```
...
ssh -i <agent-key> -p 22 root@100.99.142.120 \
  "su - hermes -c \"cd /srv/forge/agentic-forge && /opt/repo-tools/venv/bin/python forge-index/query.py '<query>' --top-k 20\""
```
