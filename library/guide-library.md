...
library/
  ...
  index-library.md             # master index (regenerated by index-library.py)
...
`library/index-library.py` after each audit cycle. The script derives all
counts from the live filesystem (R11: never hardcode). To rebuild
manually: `cd library && python index-library.py`. The Auditor is the only
