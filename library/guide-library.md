...
The master index lives in `index-library.md` -- regenerated by
`scripts/index-library.py` after each audit cycle. The script derives all
counts from the live filesystem (R11: never hardcode). To rebuild
manually: `python scripts/index-library.py`. The Auditor is the only
agent authorized to regenerate the index -- it runs the script, not
