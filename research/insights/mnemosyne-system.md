...
#### Publisher correctness update (2026-09-05)

The original dedup description above stated the intended behavior, but the
implementation queried `metadata` instead of `metadata_json` and swallowed
the SQL error. Native content dedup prevented duplicate rows while refreshing
their timestamps and potentially restoring expiry. The repaired script is
deployed identically to Morpheus, Neo, and Atlas:

- **History and repeat runs:** read `metadata_json`; compare the owning
  `source_agent` plus `episodic_id`, and check exact content too. Existing
  copies, including inactive ones, are skipped without refreshing timestamps.
  Identical content within one batch is published once. Schema or malformed
  provenance errors stop the run instead of pretending history is empty.
- **Validity and provenance:** select only nonempty, unexpired,
  nonsuperseded episodes passing the existing prefix and importance filters.
  Preserve importance, veracity, and `valid_until` when copying. Propagate
  shorter source expiry without extending a stricter shared expiry.
- **Withdrawal:** mark this agent's shared copies inactive when their
  private source expires, is superseded, disappears, or leaves the allowed
  categories. Private memories and other agents' copies are untouched.
  Normal sync distributes these expiry updates. The optional importance
  threshold controls new selection, not withdrawal of existing valid copies.
- **Concurrent arrivals:** reserve the SQLite writer with `BEGIN IMMEDIATE`
  and recheck for collisions on that same connection before insertion. A
  copy arriving through sync after the planning snapshot cannot be revived
  by the native duplicate-update path. New writes are read back and verified.
- **Operation:** the existing six-hour `no_agent` jobs and five-minute sync
  schedules are unchanged. `--dry-run` reports planned changes without writes
  or private-content logging. Publishing copies existing summaries; it does
  not create them or call Astra. Category prefixes and the `Distilled` label
  alone do not prove that an LLM produced a summary.

Publication remains a snapshot, not continuous text mirroring. To replace
a claim, invalidate/supersede its private source and create a corrected
episode. Before hard-deleting a shared copy, invalidate its private source:
deleting the shared row removes the marker that prevents republication.

Verification -- PASS or HALT:

- PASS when the regression suite and installed-Mnemosyne integration test
  pass, including repeat runs, invalid sources, content collisions, concurrent
  arrivals, and private/unrelated-row preservation; otherwise HALT deployment.
  The maintained source and tests are in Morpheus's workspace `scripts/`.
- PASS when live reruns change no rows or timestamps and natural sync carries
  the intended validity changes to all named recipients; otherwise HALT a
  health claim. Both checks passed for this repair. The next natural
  six-hour publisher execution was not part of that immediate verification.
